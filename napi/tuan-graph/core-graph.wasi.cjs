...
let __wasmFilePath = __nodePath.join(__dirname, 'core-graph.wasm32-wasi.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'core-graph.wasm32-wasi.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  try {
    __wasmFilePath = __nodePath.resolve('@tuan/core-graph-wasm32-wasi')
  } catch {
    throw new Error('Cannot find core-graph.wasm32-wasi.wasm file, and @tuan/core-graph-wasm32-wasi package is not installed.')
  }
}
